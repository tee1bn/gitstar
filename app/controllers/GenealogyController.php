...
EL;		
		...
		<img src="$image_src" style="border-radius: 70%;height: 50px; object-fit:contain;"><br>
		<b style="text-transform:capitalize;">$username </b>

		</span>
		  <div class="dropdown-menu" style="padding:0px; border-radius:5px;" aria-labelledby="dropdownMenuButton$i">
		  	$view

			<a class="text-primary" href="$domain/genealogy/placement/$user->username/binary/2" style="font-size:12px; margin-left:30px;">See Binary Tree</a>

			<a class="dropdown-tem text-primary" href="$domain/genealogy/placement/$user->username/binary/2" 
			style="font-size:12px;float:right; margin-right:60px;">Next</a>
		  </div>
		</div>
...
public function placement_list($username=null, $level_of_referral=1, $tree_key='placement', $requested_depth=null)
{
	...
	$this->view('auth/placement-structure-list', compact('list', 'user','per_page', 'level_of_referral','tree','user_column','tree_key','levels'));
